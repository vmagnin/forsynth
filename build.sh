...
# Compiling modules:
"${FC}" ${flags} -c src/forsynth.f90 src/tape_recorder_class.f90 src/wav_file_class.f90 src/envelopes.f90 src/signals.f90 src/music_common.f90 src/music.f90 src/acoustics.f90 src/audio_effects.f90 src/morse_code.f90 src/sonification.f90

# Compiling examples:
for file in "chords_and_melody" "demo_effects" "blues" "all_signals" "drum_machine" "misc_sounds" "drone_music" "shepard_scale" "shepard_risset_glissando" "doppler_effect" "arpeggios" "radioactivity" "multiplication_bells" "four_thirty_three"; do
  echo "${file}"
