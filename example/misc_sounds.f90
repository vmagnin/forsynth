        ...
        do i = nint(t1*RATE), nint(t2*RATE)-1
            select case (choice)
                ...
                case (1) ! Science fiction signal...
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(omega*t + phi*cos(75*t)/(1+log(t/100+0.01_wp)))
                case (2) ! Hummmmmmmmmm...
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(omega*t + phi*sin(omega*0.2_wp*t))
                case (3) ! UFO or siren?
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(omega*t + phi*cos(25*t))
                case (4) ! Noisy science fiction
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(omega*(1._wp + 0.2_wp*sin(t*50)) * t + phi)
                case (5) ! Whistling, slower and slower...
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(omega*(1._wp + 0.2_wp*sin(sqrt(t)*50)) * t + phi)
                case (6) ! Similar
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(omega*(1._wp + 0.2_wp*sin(log(t)*50)) * t + phi)
                case (7) ! Dudududududu...
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(t*30) * sin(omega * t + phi)
                case (8) ! Duuuuuuuuuuuuuuuuuuu
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(omega*t + phi*(0.5_wp+0.5_wp*sin(t*500)))
                case (9) ! A higher duuuuuuuuuuuuuuuuuuu
                    tape%left(track, i) = tape%left(track, i) + Amp/2 * sin(omega*t + phi) + Amp/2 * sin(omega*1.1892_wp*t + phi)
                ...
                case (12) ! Vibrato
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(omega*t + phi*(1._wp + 0.5_wp*sin(2*PI*4*t)))
                case (13) ! A mix
                ...
                case (14) ! Tremolo
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(omega*t + phi) + Amp * sin(1.001_wp*omega*t + phi) &
                ...
                case (16) ! Science fiction, becoming higher and noisy
                    tape%left(track, i) = tape%left(track, i) + Amp * sin(omega*(1._wp + 0.001_wp*sin(t*500)) * t + phi)
                case (17) ! Dissonant
                ...
                case (20) ! Another dampening sound
                    tape%left(track, i) = tape%left(track, i) + Amp*(exp(-t)*sin(omega*t) + 0.5_wp*exp(-t**2)*sin(2*omega*t) &
                ...
                case (22) ! Clarinet
                    do j=1, 11, +2
                        tape%left(track, i) = tape%left(track, i) + Amp/j**0.7_wp * sin(j*omega*t)
                    end do
