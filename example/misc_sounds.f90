...
contains
    ...
    subroutine add_misc_signal(tape, track, t1, t2, f, Amp, choice)
        ...
        do i = nint(t1*RATE), nint(t2*RATE)-1
            select case (choice)
                case (0) ! Pure sinus
                    tape%left(track, i) = + Amp * sin(omega*t)
                case (1) ! Science fiction signal...
                    tape%left(track, i) = + Amp * sin(omega*t + cos(75*t)/(1+log(t/100+0.01_wp)))
                case (2) ! Hummmmmmmmmm...
                    tape%left(track, i) = + Amp * sin(omega*t + sin(omega*0.2_wp*t))
                case (3) ! UFO or siren?
                    tape%left(track, i) = + Amp * sin(omega*t + cos(25*t))
                case (4) ! Noisy science fiction
                    tape%left(track, i) = + Amp * sin(omega*(1._wp + 0.2_wp*sin(t*50)) * t)
                case (5) ! Whistling, slower and slower...
                    tape%left(track, i) = + Amp * sin(omega*(1._wp + 0.2_wp*sin(sqrt(t)*50)) * t)
                case (6) ! Similar
                    tape%left(track, i) = + Amp * sin(omega*(1._wp + 0.2_wp*sin(log(t)*50)) * t)
                case (7) ! Dudududududu...
                    tape%left(track, i) = + Amp * sin(t*30) * sin(omega * t)
                case (8) ! Duuuuuuuuuuuuuuuuuuu
                    tape%left(track, i) = + Amp * sin(omega*t + 0.5_wp+0.5_wp*sin(t*500))
                case (9) ! A higher duuuuuuuuuuuuuuuuuuu
                    tape%left(track, i) = + Amp/2 * sin(omega*t) + Amp/2 * sin(omega*1.1892_wp*t)
                case (10) ! Higher and higher...
                    tape%left(track, i) = + Amp * sin((omega*(1+t/10)) * t)
                case (11) ! Dampening slowly
                    do j=1, 7
                        tape%left(track, i) = + Amp/(j*(1+t**j))*sin(j*omega*t)
                    end do
                case (12) ! Vibrato
                    tape%left(track, i) = + Amp * sin(omega*t + 1._wp + 0.5_wp*sin(2*PI*4*t))
                case (13) ! A mix
                    tape%left(track, i) = + Amp * sin(t) * sin(omega*t) &
                                   & + Amp * cos(2.5_wp*t) * sin(1.5_wp*omega*t) + Amp * sin(3*t) * sin(2*omega*t)
                case (14) ! Tremolo
                    tape%left(track, i) = + Amp * sin(omega*t) + Amp * sin(1.001_wp*omega*t) &
                                   & + Amp * sin(0.999_wp*omega*t)
                case (15) ! Poke... (a short percussion based on the Sinc function)
                    if (omega*t /= 0._wp) then
                        tape%left(track, i) = + Amp * sin(omega*t) / (omega*t)
                    else
                        tape%left(track, i) = + Amp
                    end if
                case (16) ! Science fiction, becoming higher and noisy
                    tape%left(track, i) = + Amp * sin(omega*(1._wp + 0.001_wp*sin(t*500)) * t)
                case (17) ! Dissonant
